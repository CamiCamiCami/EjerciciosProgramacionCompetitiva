...
void set() {
    set<int> s;
    s.insert();       // Añade elementos. Si ya existe, es ignorado.
    s.erase();        // Elimina un elemento
    s.empty();        // Retorna true (1) si el set no tiene elementos, false (0) si
                      // tiene
    s.count();        // Como un set NO tiene duplicados, solo puede retornar 1 o 0.
                      // Se usa mucho para chequear existencia.
    s.lower_bound();  // Retorna iterador al PRIMER elemento que sea MAYOR O
                      // IGUAL a 'x'.
}
...
void map() {
    map<int, int> m;
    m.insert({k, v});  // Toma: pair<K, V>. Devuelve: pair<iterator, bool>
                       // (iterador al elemento y true si se insertó nuevo).
    m.at(k);           // Toma: clave K. Devuelve: referencia al valor V. (Lanza excepción
                       // si 'k' no existe, más lento que []).
    m.contains(k);     // Toma: clave K. Devuelve: bool (true si existe. Solo
                       // disponible desde C++20).
    m.lower_bound(k);  // Toma: clave K. Devuelve: iterador al 1er elemento con
                       // clave >= k (o m.end()).
